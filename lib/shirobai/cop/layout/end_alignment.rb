...
module Shirobai
  module Cop
    module Layout
      ...
      #
      # Rust walks the AST once, reproducing stock's callback dispatch: the
      # `CheckAssignment` `on_lvasgn` / `on_send` family resolves a conditional
      # on the RHS of an assignment (following the call chain and unwrapping
      # `begin`/`||`/`&&`) and checks it against the assignment LHS
      # (`variable`), while `on_if` / `on_while` / `on_case` / `on_class` / ...
      # check a bare construct against its own keyword. Per checked `end` it
      ...
      class EndAlignment < RuboCop::Cop::Base
        ...
        # defaults to `keyword` (0) when the config does not mention this cop.
        ...
        def records_for_source
          Dispatch.offenses_for(processed_source, config, :end_alignment)
        end
