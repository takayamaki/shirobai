...
| **`ForwardingSuperNode`'s `block` field is concretely typed**: the generated walker calls `visit_block_node` directly, so a bare `super { }` block never reaches the branch enter hooks (same family as the RescueNode row) | Handle `ForwardingSuperNode` in `enter` (`ENTER_SUPER`) and process its `block()` there |
