...
/// | 130 | space_around_equals_in_parameter_default style (`Layout/SpaceAroundEqualsInParameterDefault` `EnforcedStyle`: 0 = space, 1 = no_space) — renumbered from 127 so ExtraSpacing keeps 127-129 |
///
...
impl BundleConfig {
    ...
    pub fn from_packed(
        ...
        Ok(BundleConfig {
            ...
            // Renumbered from 127 to 130 in the ExtraSpacing merge: ExtraSpacing
            // holds nums 127-129, so this style lands at the next free index.
            space_around_equals_in_parameter_default:
                space_around_equals_in_parameter_default::Config {
                    style: nums[130] as u8,
                },
...
mod tests {
    ...
    fn default_packed() -> (Vec<Vec<i64>>, Vec<Vec<Vec<String>>>) {
        let nums = vec![
            ...
            1, 1, 0, // extra_spacing: enabled / allow_for_alignment(true) / allow_before_trailing_comments(false) — indices 127-129
            0, // space_around_equals_in_parameter_default: style (space) — index 130
        ];
        let lists = vec![
