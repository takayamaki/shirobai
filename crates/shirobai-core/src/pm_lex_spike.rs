//! SPIKE (hypothesis check, not production): raw prism parse + in-line lex.
//!
//! Goal: get the same in-memory AST that `ruby_prism::parse()` produces AND
//! the token stream from the *same single parse*, without prism's old
//! `pm_serialize_lex` tax (a second full parse).
//!
...
//! a callback).
//!
//! The transmute is the load-bearing bet; `assert_parse_result_layout()` and the
//! canary tests exist to catch it if the layout assumption ever breaks.
...
/// the `lex_callback` wiring in between init and parse.
///
/// # Safety
...
///
/// # Safety
/// Same contract as [`parse_with_lex`].
#[must_use]
...
mod tests {
    ...
    fn collects_tokens_matching_a_known_snippet() {
        ...
        let named: Vec<(&str, usize, usize, u32)> = tokens
            ...
            .collect();
        // From `Prism.lex` on the same source (see spike report).
