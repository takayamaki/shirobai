...
/// Standalone entry point for `RSpec/DescribedClass` (candidate block ranges).
fn check_rspec_described_class(
    ruby: &Ruby,
    source: RString,
    nums: Vec<i64>,
    lists: Vec<Vec<String>>,
    ...
    Ok(shirobai_core::rules::rspec_dispatcher::check_rspec_described_class(
        bytes(&source),
        &cfg,
    ))
}
...
/// Standalone entry point for `RSpec/MultipleSubjects` (per-cop fallback).
/// Returns overwritten-subject block ranges `[[block_start, block_end], ...]`.
fn check_rspec_multiple_subjects(
    ruby: &Ruby,
    source: RString,
    nums: Vec<i64>,
    lists: Vec<Vec<String>>,
    ...
    Ok(shirobai_core::rules::rspec_dispatcher::check_rspec_multiple_subjects(
        bytes(&source),
        &cfg,
    ))
}

/// Standalone entry point for `RSpec/SharedExamples` (per-cop fallback).
/// Returns candidate send ranges `[[send_start, send_end], ...]`.
fn check_rspec_shared_examples(
    ruby: &Ruby,
    source: RString,
    nums: Vec<i64>,
    lists: Vec<Vec<String>>,
    ...
    Ok(shirobai_core::rules::rspec_dispatcher::check_rspec_shared_examples(
        bytes(&source),
        &cfg,
    ))
}
...
fn check_all(ruby: &Ruby, source: RString, token: usize) -> Result<RArray, Error> {
    BUNDLE_CONFIGS.with(|cell| {
        ...
        let rspec = ruby.ary_new_capa(25);
        ...
        rspec.push(r.rspec_multiple_subjects)?;
        // slot 24: RSpec/SharedExamples (candidate send ranges).
        rspec.push(r.rspec_shared_examples)?;
...
fn init(ruby: &Ruby) -> Result<(), Error> {
    ...
    module.define_module_function(
        "check_rspec_multiple_subjects",
        function!(check_rspec_multiple_subjects, 3),
    )?;
    module.define_module_function(
        "check_rspec_shared_examples",
        function!(check_rspec_shared_examples, 3),
    )?;
    module.define_module_function(
