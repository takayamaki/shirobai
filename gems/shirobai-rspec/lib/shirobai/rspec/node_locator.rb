...
module Shirobai
  module RSpec
    ...
    # `SourceOffsets` first), `locate` returns a Hash mapping each requested
    # `[begin, end]` to the node whose expression range matches it exactly.
    #
    ...
    #    lives inside its ancestors' ranges, so such a subtree cannot hold
    #    one. The walk cost is then proportional to the paths down to the
    #    targets, not the whole AST. Record exact-range matches shallowest-wins
    #    and early-exit once every target is found. A wrapper sharing its
    #    child's range still has that range CONTAIN the target range, so the
    #    prune never skips it — shallowest-wins survives pruning.
    #
    ...
    #    pre-order descent (no pruning) to locate the stragglers, again with
    #    the early-exit. Prune alone is unsound (drops heredoc-interior
    #    targets); full descent alone is slow on large spec files (walks the
    #    whole AST even when targets sit late); the fallback pays the full walk
    #    only when a target actually hides in a heredoc.
    #
    ...
    module NodeLocator
      ...
      def locate(processed_source, ranges)
        ...
        # Phase 2: only if a target escaped phase 1 (heredoc interior), pay for
        # a full descent to find whatever is still missing.
      ...
      # and unseen, then recurse only into children whose range could still
      # contain a remaining target. Returns `true` once all targets are found so
      # callers can unwind the descent immediately.
      ...
      # Pre-order visit `node` with NO pruning; record it if its expression
      # range is a target and unseen, then recurse into every child. Returns
      # `true` once all targets are found so callers can unwind the descent
      # immediately. This is the sound-but-slow path used only for targets
